            ...
            <Card className="glass-morphism p-12 rounded-3xl border-gray-600">
                ...
                >
                  <h3 className="text-4xl font-bold gradient-text mb-2 transition-all duration-500">
                    {statsLoading ? '...' : formatTraderCount(platformStats?.activeTraders || 1)}
                  </h3>
                  <p className="text-gray-300">Active Traders</p>
                  {platformStats?.recentUsers > 0 && (
                    <p className="text-xs text-green-400 mt-1">+{platformStats.recentUsers} this week</p>
                  )}
                </motion.div>
                <motion.div
                  initial={{ opacity: 0, y: 30 }}
                ...
                >
                  <h3 className="text-4xl font-bold gradient-text mb-2 transition-all duration-500">
                    {statsLoading ? '...' : formatLargeNumber(platformStats?.trackedVolume || 0)}
                  </h3>
                  <p className="text-gray-300">Tracked Volume</p>
                  {platformStats?.totalTrades > 0 && (
                    <p className="text-xs text-blue-400 mt-1">{platformStats.totalTrades} trades</p>
                  )}
                </motion.div>
                <motion.div
                  initial={{ opacity: 0, y: 30 }}
                ...
                >
                  <h3 className="text-4xl font-bold gradient-text mb-2">
                    {statsLoading ? '...' : `${platformStats?.successRate?.toFixed(1) || '0.0'}%`}
                  </h3>
                  <p className="text-gray-300">Success Rate</p>
                  {platformStats?.totalTrades > 0 && (
                    <p className="text-xs text-gray-400 mt-1">Real trader data</p>
                  )}
                </motion.div>
                <motion.div
                  initial={{ opacity: 0, y: 30 }}
                ...
                >
                  ...
                  {platformStats?.recentTrades > 0 && (
                    <p className="text-xs text-green-400 mt-1">{platformStats.recentTrades} trades today</p>
                  )}
                </motion.div>
              </div>
            </Card>
